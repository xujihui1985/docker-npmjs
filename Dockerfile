...
RUN mkdir -p /usr/local/var/log/couchdb && touch /usr/local/var/log/couchdb/couch.log
...
RUN cd /opt/npmjs; couchdb -b; sleep 5; curl http://isaacs.iriscouch.com/registry/error%3A%20forbidden | curl -X PUT -d @- http://localhost:5984/registry/error%3A%20forbidden?new_edits=false; sleep 5; couchdb -d
